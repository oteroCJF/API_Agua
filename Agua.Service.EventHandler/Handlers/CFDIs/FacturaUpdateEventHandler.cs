...
namespace Agua.Service.EventHandler.Handlers.CFDIs
{
    public class FacturaUpdateEventHandler : IRequestHandler<FacturaUpdateCommand, Factura>
    {
        ...
        public async Task<Factura> Handle(FacturaUpdateCommand request, CancellationToken cancellationToken)
        {
            ...
            Factura xmlExists = null;
            if (file.Contains("_"))
            {
                var tipo = file.Split("_")[0];
                xmlExists = _context.Facturas.SingleOrDefault(f => f.ArchivoXML.Contains(file) && f.Tipo.Equals(tipo));
            }
            else
            {
                xmlExists = _context.Facturas.SingleOrDefault(f => f.ArchivoXML.Contains(file));
            }
